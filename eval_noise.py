"""
Script to evaluate Jin et al. CVPR 2018 model with Gaussian noise added to inputs.

This script:
1. Loads GoPro test sequences with pre-existing blur images
2. Downsamples to 45% resolution
3. Adds Gaussian noise to test robustness
4. Runs the model inference using their actual model architecture
5. Calculates PSNR for middle frame prediction

Usage:
    python eval_noise.py --gopro_path /path/to/GOPRO_Large/test --cuda --noise_sigma 10
"""
...
def add_gaussian_noise(tensor, sigma):
    ...
    Args:
        ...
        sigma: Standard deviation of Gaussian noise (in range [0, 255])
    
    Returns:
        Noisy tensor in range [0, 1]
    """
    # Scale sigma to [0, 1] range since tensor is normalized
    sigma_normalized = sigma / 255.0
    
    # Generate Gaussian noise
    noise = torch.randn_like(tensor) * sigma_normalized
...
def evaluate_middle_frame(models, samples, cuda=True, noise_sigma=0):
    """
    ...
    Args:
        ...
        cuda: Whether to use CUDA
        noise_sigma: Gaussian noise sigma (0-255 scale, 0 = no noise)
    ...
    for idx, sample in enumerate(tqdm(samples)):
        ...
        # ADD GAUSSIAN NOISE
        if noise_sigma > 0:
            blurry_input = add_gaussian_noise(blurry_input, noise_sigma)
...
def save_results(results, output_dir='evaluation_results', noise_sigma=0):
    ...
    # Calculate drop from baseline if noise was added
    if noise_sigma > 0:
        baseline_psnr = 32.20  # Paper's reported baseline on their test set
        drop = baseline_psnr - results['mean_psnr']
        print(f"PSNR drop from baseline (32.20 dB): {drop:.4f} dB")
    ...
    with open(os.path.join(output_dir, 'psnr_results.txt'), 'w') as f:
        f.write(f"Gaussian noise sigma: {noise_sigma}\n")
        ...
        if noise_sigma > 0:
            drop = 32.20 - results['mean_psnr']
            f.write(f"PSNR drop from baseline: {drop:.4f} dB\n")
        f.write(f"\nAll PSNR scores:\n")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate Jin et al. CVPR 2018 model on GoPro dataset with Gaussian noise')
    ...
    parser.add_argument('--noise_sigma', type=float, default=0,
                       help='Gaussian noise sigma (0-255 scale, default: 0 = no noise)')
    ...
    print(f"\n{'='*60}")
    print(f"GoPro Dataset Evaluation with Gaussian Noise")
    print(f"{'='*60}")
    ...
    # Save results
    save_results(results, args.output_dir, args.noise_sigma)
